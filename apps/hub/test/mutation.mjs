...
const MUTANTS = [
  ...
  ['cctv: Range 목록 검사 끔', 'src/cctv.ts', "if (range !== null && isForwardableRange(range)) headers.set('Range', range);", "if (range !== null) headers.set('Range', range);"],
  ["cctv: basic 아이디에 ':' 허용", 'src/cctv.ts', "!CRED_RE.test(user) || user.includes(':')", "!CRED_RE.test(user)"],
  ['cctv: 준비 판정에서 인증 설정 무시', 'src/cctv.ts', "return relayCheck(env).ok;", "return isEnabled(env) && relayOrigin(env) !== null;"],
  ...
  ['cctv: 카메라 이름 규칙 검사 끔', 'src/cctv.ts', "return CAMERA_ID(v, 'camera_id');", 'return v;'],
  ['cctv: 머리말 대기 시간 제한 끔', 'src/cctv.ts', "const timer = setTimeout(() => ctl.abort(), deps.connectTimeoutMs ?? UPSTREAM_TIMEOUT_MS);", 'const timer = setTimeout(() => undefined, 0);'],
  ['admin: 방식 변경 시 경로 필수 검사 끔', 'src/admin.ts', "if (body.stream_path === undefined && before !== null && before.stream_path !== null && before.stream_kind !== body.stream_kind) {", 'if (false) {'],
  ['validate: 정수 범위 검사 끔 (카메라 정렬값)', 'src/validate.ts', "if (typeof v !== 'number' || !Number.isInteger(v) || v < opts.min || v > opts.max) {", 'if (false) {'],
  ['admin: 경로를 빼면 이전 경로 대신 미연결로 저장', 'src/admin.ts', "body.stream_path === undefined ? (before?.stream_path ?? null) : body.stream_path;", "body.stream_path === undefined ? null : body.stream_path;"],
  ['cctv: 구간 시작>끝 허용', 'src/cctv.ts', "if (start !== '' && end !== '' && Number(start) > Number(end)) return false;", ''],
  ['cctv: 압축 응답 허용', 'src/cctv.ts', "if (enc !== '' && enc !== 'identity') {", 'if (false) {'],
  ['cctv: 몸통 멈춤 감시 끔', 'src/cctv.ts', "withIdleTimeout(upstream.body, deps.idleTimeoutMs ?? IDLE_TIMEOUT_MS, abortOf(ctl))", 'upstream.body'],
  ['cctv: 416 을 502 로 위장', 'src/cctv.ts', "if (upstream.status === 416) {", 'if (false) {'],
