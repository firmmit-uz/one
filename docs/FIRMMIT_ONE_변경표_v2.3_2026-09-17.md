...
## B-8. 코드 리뷰 6차 (Z101–Z109)

`/code-review`(high) 6차 9건 → 전부 수정.

| ID | 위치 | 상태 | 내용 |
|---|---|---|---|
| Z101 | `public/app.js` | 수정함 | 재열람 시점을 서버 절대 시각이 아니라 **수명(`expires_at − opened_at`)을 받은 시각에 더해** 잰다 — 브라우저 시계가 어긋나면 1초마다 재열람(감사기록 폭주)하거나 만료 뒤에야 열던 것. 최소 간격 1분 |
| Z102 | `src/cctv.ts` | 수정함 | 멈춤 감시를 변환 스트림 훅이 아니라 **위쪽 읽기마다** 잰다 — 브라우저가 일시정지(역압)한 것을 중계 서버 멈춤으로 오인해 끊던 것 |
| Z103 | `src/admin.ts` `public/app.js` `i18n.js` | 수정함 | 카메라 저장에서 `stream_path` **생략 = 이전 경로 유지**, `null` = 미연결, 새 등록 생략 = 미연결. 목록이 경로를 돌려주지 않으므로 이름·순서만 고치는 저장이 경로를 지우던 것. 폼에 「미연결로 저장」 확인란 |
| Z104 | `public/app.js` | 수정함 | 재청 1.5초·3초 간격, 만료 전 실패가 계속되면 `/open` 을 한 번 더 불러 상태(토큰·카메라)를 새로 확인 — 즉시 3연타·409 무한 실패 |
| Z105 | `scripts/dev-seed.sql` | 수정함 | 없는 `CCTV_ON=1` 안내 → 실제 스위치(`CCTV=off`, 기본 켜짐) |
| Z106 | `README.md` | 수정함 | "정전이면 화면이 502" 는 `/open` 때만 참 — 보던 중 끊김의 실제 동작(재청 2회·재열람 1회·안내)으로 정정 |
| Z107 | `public/i18n.js` | 수정함 | CCTV 표 동작 열 머리글 `''` → `동작` (보조기술·휴대폰 카드에 이름 없던 것) |
| Z108 | `src/validate.ts` `src/admin.ts` | 수정함 | 정수 범위 검사 `int({min,max})` 를 공용 검증기로 (변이는 `validate.ts` 를 겨눔) |
| Z109 | `public/app.js` | 수정함 | `errorText()` 의 ko 되돌림을 `t()` 하나로 (규칙 두 곳 중복 제거) |

시험: hub **311/311** · 변이 **57/57** 미검출 0 · typecheck 0 · 화면 18장 + 흐름 3 실패 0 · 번들 622.0 KiB.

## B-9. 코드 리뷰 7차 (Z110–Z117)

`/code-review`(high) 7차 8건 → 전부 수정.

| ID | 위치 | 상태 | 내용 |
|---|---|---|---|
| Z110 | `public/app.js` | 수정함 | 재열람·재청 예약 타이머를 **한 개**로 — 새로 걸기 전에 반드시 지운다. 지우지 않으면 닫거나 다른 카메라로 바꾼 뒤에도 옛 예약이 살아 옛 카메라를 다시 끼워 넣고 열람 기록을 남기던 것 |
| Z111 | `public/app.js` | 수정함 | `/open` 을 기다리는 동안 화면을 떠났거나 닫기를 눌렀으면 시작하지 않는다 — 떼어진 `<video>` 도 자동재생으로 중계 연결을 열어 최대 15분 회선을 차지하던 것 |
| Z112 | `src/admin.ts` | 수정함 | 재생 방식을 바꾸면서 경로를 빼면 **거부** — 옛 `.mp4` 경로를 사진 방식에 물려주면 목록은 "볼 수 있음" 인데 재생은 형식 불일치로 늘 막히는 어긋남 |
| Z113 | `src/cctv.ts` `src/app.ts` `test/*` | 수정함 | 머리말 대기 10초 제한에 시험·변이가 없었다 → 대기 시간을 주입 가능하게 하고 "영원히 안 주는 중계" 시험 + 변이 추가 |
| Z114 | `public/app.js` | 수정함 | mp4 는 토큰 갱신 때 재생기를 다시 만들지 않는다 — 진행형 스트림은 요청 시작 때만 토큰을 보므로 15분마다 화면이 끊기고 열람 기록이 늘던 것. 새 토큰은 다음 오류 때부터 쓴다 |
| Z115 | `src/cctv.ts` | 수정함 | 멈춤 감시가 조각마다 Promise·클로저를 만들지 않고 타이머 하나만 건다 |
| Z116 | `src/cctv.ts` | 수정함 | 416 도 `reject(status)` 한 길로 — 본문 취소·던지기 중복 제거 (변이가 두 곳 중 한 곳만 겨누던 것) |
| Z117 | `src/cctv.ts` | 수정함 | 경로 매개변수의 카메라 이름 검사를 본문과 같은 `str({min,max,pattern})` 으로 — "올바른 이름" 정의가 두 곳으로 갈라지지 않게 |

시험: hub **313/313** · 변이 **59/59** 미검출 0 · typecheck 0 · 화면 18장 + 흐름 3 실패 0 · 번들 622.5 KiB.

## B-10. 코드 리뷰 8차 (Z118–Z125)

`/code-review`(high) 8차 8건 → 7건 수정, 1건은 근거를 기록.

| ID | 위치 | 상태 | 내용 |
|---|---|---|---|
| Z118 | `public/app.js` | 수정함 | 열기 흐름에 **세대 번호** — 카메라 A 의 늦은 `/open` 응답이나 옛 예약이 지금 보는 카메라 B 를 덮거나 B 의 예약 타이머를 훔치던 경쟁 |
| Z119 | `src/cctv.ts` | 수정함 | 멈춤 감시의 끊기 클로저가 `proxyStream` 지역(접속표가 든 `headers` 등)을 스트림 수명(몇 시간)만큼 붙들지 않게 `abortOf(ctl)` 로 분리 |
| Z120 | `src/admin.ts` | 수정함 | 관리 목록을 `views[i]`·`rows[i]` 인덱스로 짝짓지 않고 행에서 직접 만든다 |
| Z121 | `src/admin.ts` | 수정함 | 저장할 행을 `resolveCameraWrite()` 한 번으로 계산해 **문장과 응답이 같은 값** — bind 9칸과 응답을 손으로 두 번 맞추던 것 |
| Z122 | `src/cctv.ts` | 수정함 | `relayCheck(env)` 가 실패 코드를 주고 `relayReady`·`playTarget` 이 같은 것을 쓴다 — 목록 판정과 재생 판정의 차례가 갈라질 수 있던 것 |
| Z123 | `CLAUDE.md` | 기록함 | CCTV 화면·오류 문구가 **한국어만**인 것은 FIRMMIT 지시("언어는 한국어로만, 우리 경영진만")에 따른 것 — 규칙 파일에 예외로 적었다. 코드는 그대로 |
| Z124 | `public/i18n.js` | 수정함 | 안 쓰는 키 `cctv_playing` 제거 |
| Z125 | `src/cctv.ts` | 수정함 | 소비자가 먼저 취소한 뒤 `read()` 가 `done` 으로 풀리면 닫힌 스트림에 `close()` 를 부르지 않는다 |

시험: hub **313/313** · 변이 **59/59** 미검출 0 · typecheck 0 · 화면 18장 + 흐름 3 실패 0 · 번들 623.0 KiB.
